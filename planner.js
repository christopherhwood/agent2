...
async function generateRoughPlan(taskDescription, summary) {
  const systemPrompt = `You are a software development strategist, skilled in translating complex codebase insights into actionable coding plans with a keen eye on the end user experience. Your task is to meticulously examine relevant files and commits from a Git repository, considering the nuances of the project's coding style, structure, best practices in JavaScript, and the impact on the end user. Just focus on the code-writing and DO NOT worry about or include steps for setting up the environment, cloning the repo, committing changes, deployment, code review, documentation, or what happens after the code is written. Utilize the detailed information provided from the repository, such as the provided summary and Git blame and history data, to understand the evolution and current state of the codebase. Incorporate details from the summary and git blame and history data into your plan. 
  
  DO NOT create tasks to read the code that is already provided. Instead, distill that information into the plan itself.

  DO NOT create catch-all tasks like 'Implement logging and error handling',  'Enhance end user experience', or 'Conduct Final Review'. Instead, include these as specific steps to other tasks including specifically what should be logged or what to be cautious of in terms of user experience or thoughts for how to improve the user experience. If you deem certain steps to be trickier then include instructions on what specifically to review. For example, 'Review the code for any potential bugs or errors' is not a good task. Instead, 'Review the code for any potential bugs or errors in the following areas: [list of areas]' is a better task. Remember that your goal is to write as detailed of tasks as possible so that the developer can just follow the steps and not have to think about what to do next. It's okay (and even preferred) for descriptions to be long and detailed.
...
async function generateTaskTree(taskDescription, summary, roughPlan) {
  ...
  Make the task descriptions as detailed as possible, including specific instructions on what to do, what to look for, and what to avoid. For example, instead of 'Implement logging and error handling', include specific instructions on what specifically to log or what to be cautious of in terms of user experience or thoughts for how to improve the user experience. Remember that your goal is to write as detailed of tasks as possible so that the developer can just follow the steps and not have to think about what to do next. It's okay (and even preferred) for descriptions to be long and detailed.
