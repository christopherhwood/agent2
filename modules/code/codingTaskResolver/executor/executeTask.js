...
async function executeTask(task, problemStatement, coder) {
  ...
  spec.task = task;
  console.log('spec:\n', spec);
  // Go through list of files to add/edit/delete and prompt for each action.
  for (const filePath of spec.filesToAdd) {
    const res = await addFile(filePath, spec, coder.repoName);
...
//   const messages = await queryLlmWTools([{role: 'system', content: ExecuteTaskSystemPrompt}, {role: 'user', content: query}], coder.getTools(), coder);
